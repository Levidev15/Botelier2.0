...
END $$""",
    ...
    ("Drop old index ix_call_logs_hotel_status", "DROP INDEX IF EXISTS ix_call_logs_hotel_status"),
    (
        "Create ix_call_logs_account_status",
        "CREATE INDEX IF NOT EXISTS ix_call_logs_account_status ON call_logs(account_id, status)",
    ),
    ("Drop old index ix_sms_conv_hotel_status", "DROP INDEX IF EXISTS ix_sms_conv_hotel_status"),
    (
    ...
    ("Drop old index ix_sms_template_hotel", "DROP INDEX IF EXISTS ix_sms_template_hotel"),
    (
    ...
    ("Drop legacy hotels table", "DROP TABLE IF EXISTS hotels"),
]
...
cur2.execute(
    "SELECT table_name FROM information_schema.tables WHERE table_name = 'hotels' AND table_schema = 'public'"
)
