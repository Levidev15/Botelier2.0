...
from botelier.api.payment_pages import router as payment_pages_router
...
from botelier.api.capabilities import router as capabilities_router
...
from botelier.api.email_senders import router as email_senders_router
...
app.include_router(email_senders_router)  # Email sender connections (Gmail / Microsoft) — before integrations_router
...
app.include_router(payment_pages_router)  # Per-property review+pay page designer (Task #339)
...
async def shutdown_event():
    ...
    # The actual finalizer lives in `botelier.services.shutdown_finalizer`
    # so it can be unit-tested without importing this module's heavy
    # FastAPI/pipecat graph.
    try:
