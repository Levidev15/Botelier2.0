    ...
    for group in groups:
        try:
            ...
            if not dry_run:
                ...
                if survivor:
                    if group["max_duration"] and group["max_duration"] > (
                        survivor.duration_seconds or 0
                    ):
                        survivor.duration_seconds = group["max_duration"]
                    if group["first_started"] and (
                        not survivor.started_at or group["first_started"] < survivor.started_at
                    ):
                        survivor.started_at = group["first_started"]
                    if group["last_ended"] and (
                        not survivor.ended_at or group["last_ended"] > survivor.ended_at
                    ):
                ...
                deleted = (
                    db.query(CallLeg)
                    .filter(CallLeg.id.in_(duplicate_ids))
            ...
            logger.info(
                f"{'[DRY-RUN] ' if dry_run else ''}Processed call {group['call_sid']}: "
...
def renumber_legs(db: Session, call_log_ids: list, dry_run: bool = True) -> int:
    ...
    for call_log_id in call_log_ids:
        legs = (
            db.query(CallLeg)
            .filter(CallLeg.call_log_id == call_log_id)
            .order_by(CallLeg.leg_number, CallLeg.created_at)
            .all()
        )
...
def main():
    ...
    logger.info(
        f"Mode: {'DRY-RUN (preview only)' if dry_run else 'EXECUTE (will modify database)'}"
    )
    logger.info(f"{'=' * 60}")
    ...
    try:
        ...
        logger.info(
            f"Found {len(groups)} calls with duplicate transfer legs ({total_duplicates} duplicates total)"
        )
        ...
        for i, g in enumerate(groups[:10], 1):
            logger.info(
                f"  {i}. Call {g['call_sid']}: {g['leg_count']} legs to {g['participant']} "
