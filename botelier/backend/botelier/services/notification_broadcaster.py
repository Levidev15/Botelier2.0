...
Architecture:
    ...
    - When a new SMS arrives, the webhook calls broadcast() which puts an event
      into every registered queue for that hotel — zero HTTP requests, instant delivery.
...
Future scaling note:
    - For multi-process / multi-worker deployments, replace the in-memory dict
      with a Redis pub/sub channel (one channel per hotel_id). The interface
...
class NotificationBroadcaster:
    ...
    Usage:
        ...
        queue = broadcaster.subscribe(hotel_id)           # SSE endpoint
        await broadcaster.broadcast(hotel_id, "new_message", {...})  # webhook
        broadcaster.unsubscribe(hotel_id, queue)          # on disconnect
    """

    def __init__(self) -> None:
        self._subscribers: Dict[str, Set[asyncio.Queue]] = defaultdict(set)

    def subscribe(self, hotel_id: str) -> asyncio.Queue:
        """Register a new SSE client for hotel_id. Returns its event queue."""
        q: asyncio.Queue = asyncio.Queue(maxsize=MAX_QUEUE_SIZE)
        self._subscribers[hotel_id].add(q)
        logger.debug(f"SSE client subscribed — hotel {hotel_id} "
                     f"({len(self._subscribers[hotel_id])} connected)")
        return q

    def unsubscribe(self, hotel_id: str, queue: asyncio.Queue) -> None:
        """Remove a client queue. Called when the SSE connection closes."""
        self._subscribers[hotel_id].discard(queue)
        if not self._subscribers[hotel_id]:
            del self._subscribers[hotel_id]
        logger.debug(f"SSE client disconnected — hotel {hotel_id} "
                     f"({len(self._subscribers.get(hotel_id, set()))} remaining)")

    async def broadcast(self, hotel_id: str, event_type: str, data: dict) -> None:
        """
        Push an event to all SSE clients watching hotel_id.
        ...
        """
        subscribers = self._subscribers.get(hotel_id, set())
        if not subscribers:
            return
        ...
        logger.debug(f"SSE broadcast '{event_type}' → hotel {hotel_id} "
                     f"({len(subscribers)} clients, {dropped} dropped)")

    async def event_generator(
        self, hotel_id: str, keepalive_seconds: int = 15
        ...
        try:
            while True:
                try:
                    ...
                    # SSE keepalive — comment line, not an event
                    yield {"event": "keepalive", "data": ""}
        except asyncio.CancelledError:
            pass
        finally:
            self.unsubscribe(hotel_id, queue)
