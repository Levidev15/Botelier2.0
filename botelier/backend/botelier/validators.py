...
class SlotValidators:
    ...
    @staticmethod
    def validate_date(
        value: str, require_future: bool = True, min_days_ahead: int = 0, max_days_ahead: int = 365
    ) -> ValidationResult:
        ...
        if value_lower in ["today", "tonight"]:
            ...
            if not parsed_date:
                try:
                    ...
                    for part in parts:
                        part_clean = (
                            part.rstrip(",").rstrip("st").rstrip("nd").rstrip("rd").rstrip("th")
                        )
    ...
    def validate_number(
        ...
        try:
            ...
            return ValidationResult(
                False, value, "Please provide a valid number.", ["Say a number like 2 or two"]
            )
    ...
    @staticmethod
    def validate_text(
        value: str, min_length: int = 1, max_length: int = 500, pattern: Optional[str] = None
    ) -> ValidationResult:
        ...
        if pattern:
            if not re.match(pattern, text):
                return ValidationResult(
                    False, value, "Your response doesn't match the expected format."
                )
    ...
    def validate_choice(value: str, choices: list[str]) -> ValidationResult:
        ...
        return ValidationResult(
            False, value, f"Please choose from: {choice_list}", [f"Your options are: {choice_list}"]
        )
...
class CrossFieldValidators:
    ...
    def validate_date_range(
        check_in: str, check_out: str, min_nights: int = 1, max_nights: int = 30
    ) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                False, None, "Please provide valid dates for check-in and check-out."
            )
        ...
        return ValidationResult(
            True, {"check_in": check_in, "check_out": check_out, "nights": nights}
        )

    @staticmethod
    def validate_guest_count_for_room(guest_count: int, room_type: str) -> ValidationResult:
...
def validate_slot(
    slot_type: str, value: Any, validation_config: Optional[dict] = None
) -> ValidationResult:
