...
class BoteilerFlowEngine:
    ...
    def _parse_flow_config(self, config: Dict[str, Any]) -> None:
        ...
        for node in nodes:
            ...
            self._nodes[node_id] = self._convert_to_pipecat_node(node, edge_map.get(node_id, []))

        logger.info(
            f"🔄 Parsed flow config: {len(self._nodes)} nodes for account {self.account_id}"
        )

    def _convert_to_pipecat_node(
        self, node: Dict[str, Any], next_nodes: List[str]
    ) -> Dict[str, Any]:
        ...
        if node_data.get("functions"):
            pipecat_node["functions"] = self._process_functions(node_data["functions"], next_nodes)
    ...
    def _process_functions(self, functions: List[Dict], next_nodes: List[str]) -> List[Dict]:
    ...
    def to_pipecat_config(self) -> Optional[Dict[str, Any]]:
        ...
        return {"initial_node": self._initial_node_id, "nodes": self._nodes}
