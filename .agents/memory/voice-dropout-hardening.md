...
- **Terminal actions (transfer/hangup) after an announcement must never proceed on an unconfirmed playback boundary.** Degraded paths (no mark watcher, mark-ack timeout, no completion watcher) wait a conservative length-scaled playback estimate of the announcement instead of a flat delay; the pre-speech watchdog counts TTS synthesis frames (observed at/after scheduling only — stale activity from earlier utterances must not starve it) as "speech pending" so it can't fire before the announcement becomes audible.
