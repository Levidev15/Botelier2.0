...
**Bridge lessons:** any bridge/transition line spoken via TTSSpeakFrame enters the LLM context, and small models will parrot it after ordinary turns — pair every system-spoken phrase with an explicit anti-parroting prompt rule. When gating a safety-net utterance on "something else will be spoken", the gate must mirror the actual speak conditions exactly, or callers get stranded in silence. Backend-only config knobs are incomplete: a per-node setting the flow editor cannot edit doesn't count as configurable.
