...
- `basic_or_jwt` + `basic_auth`: sends `Authorization: Basic base64(username:password)` and appends every key listed in `auth_config.basic_auth_query_params` (sourced from the stored credentials blob, falling back to `connection_config`) as a URL query parameter — e.g. `apikey` and `hotelId` for GuestCentric.
...
**Populating `connection_config` from the Connect/Edit modal (`storage` flag):** mark any entry in your type's `required_fields` with `"storage": "connection_config"` and its submitted value is routed into `connection_config` instead of the encrypted credentials blob — the Connect and Edit modals already render `required_fields` generically, so no frontend change is needed. The `/connect` and credentials `PATCH` routes split fields by this flag server-side (`_split_fields_by_storage`), the credentials `GET` merges connection_config back so the edit form pre-fills it, and on save any legacy copy of the key still sitting in the credentials blob is dropped so the two stores can't diverge. Use this for non-secret, property-level constants (GuestCentric's `hotelId` is declared this way); keep genuine secrets un-flagged so they stay encrypted. Note the key-case bridge: `_build_url` accepts both `hotel_id` and `hotelId` and substitutes all of `{hotelId}`/`{{hotelId}}`/`{hotel_id}`/`{{hotel_id}}`, so a camelCase field key still resolves a snake_case `{{hotel_id}}` path placeholder. For constants an operator must set out-of-band instead (no field on the type), populate the row directly. Do not confuse any of this with `connection_name`, which only labels the connection in the flow-editor dropdown (see Step 4).
